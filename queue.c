...
struct queue {
  ...
  pthread_cond_t delete; // condition variable used to notify threads when
                         // deleting
  pthread_mutex_t mutex; // deafult mutex used in insertion operations
  pthread_mutex_t
      err_mutex; // mutex used to ensure thread safety in try pop and push
  pthread_mutexattr_t err_attr; // atributes used for the err_mutex
};
...
Queue *q_init() {
  ...
  if (pthread_mutexattr_settype(&(q->err_attr), PTHREAD_MUTEX_ERRORCHECK)) {
    fprintf(stderr,
            "Error: mutex failed to set attr PTHREAD_MUTEX_ERRORCHECK\n");
  ...
  if (pthread_mutex_init(&(q->err_mutex), &(q->err_attr))) {
    fprintf(
        stderr,
        "Error: mutex failed to create with attr PTHREAD_MUTEX_ERRORCHECK\n");
    pthread_mutexattr_destroy(&(q->err_attr));
    free((void *)q);
    q = NULL;
    return q;
  ...
  if (pthread_mutex_init(&(q->mutex), NULL)) {
    fprintf(stderr,
            "Error: mutex failed to create with attr PTHREAD_MUTEX_DEFAULT\n");
    pthread_mutexattr_destroy(&(q->err_attr));
